...
"""
    ...
    def _format_context_with_session_data(self, context: Dict[str, Any]) -> str:
        ...
        orders_data = (
            context.get("sample_orders") or context.get("orders") or context.get("processed_orders")
        )
        if orders_data:
            count = (
                len(orders_data)
                if isinstance(orders_data, list)
                else len(orders_data.get("index", {}))
            )
            data_counts.append(f"Orders: {count}")

        # Check for inventory data
        inventory_data = (
            context.get("sample_inventory")
            or context.get("inventory")
            or context.get("inventory_summary")
        )
        if inventory_data:
            count = (
                len(inventory_data)
                if isinstance(inventory_data, list)
                else len(inventory_data.get("index", {}))
            )
        ...
        if shortage_data:
            count = (
                ...
                else len(shortage_data.get("index", {}))
            )
        ...
        if context.get("inventory_comparison"):
            comp_count = context.get(
                "inventory_comparison_count", len(context["inventory_comparison"])
            )
        ...
        if context.get("airtable_delivery_services"):
            airtable_counts.append(
                f"Delivery Services: {context.get('airtable_services_count', 0)}"
            )
        if context.get("airtable_fulfillment_centers"):
            airtable_counts.append(
                f"Fulfillment Centers: {context.get('airtable_centers_count', 0)}"
            )
        ...
        shortage_data = context.get("shortage_summary") or context.get("grouped_shortage_summary")
        if shortage_data:
            total_shortages = context.get(
                "shortage_count", len(shortage_data) if isinstance(shortage_data, list) else 0
            )
            ...
            if isinstance(shortage_data, list):
                ...
                formatted.append(
                    f"Showing first 10 of {len(shortage_data['index'])} shortage items:"
                )
        ...
        if inventory_data:
            if isinstance(inventory_data, dict) and "index" in inventory_data:
                ...
                for warehouse, data in warehouse_summary.items():
                    formatted.append(
                        f"- {warehouse}: {data['total_qty']} total units, {len(data['skus'])} different SKUs"
                    )
        ...
        if context.get("airtable_delivery_services"):
            ...
            for i, service in enumerate(
                context["airtable_delivery_services"][:10]
            ):  # Show first 10
                service_details = []
        ...
        if context.get("airtable_fulfillment_centers"):
            ...
            for i, center in enumerate(
                context["airtable_fulfillment_centers"][:10]
            ):  # Show first 10
                center_details = []
        ...
        if context.get("staging_workflow_active"):
            ...
            if context.get("currently_processing_orders") and processing_count > 0:
                ...
                for i, order in enumerate(
                    context["currently_processing_orders"][:10]
                ):  # Show first 10
                    order_details = []
                    for key in [
                        "ordernumber",
        ...
        if context.get("staging_processor_data"):
            ...
            formatted.append(
                f"Workflow Initialized: {proc_data.get('workflow_initialized', False)}"
            )
            ...
            formatted.append(
                f"Initial Inventory Items: {proc_data.get('initial_inventory_count', 0)}"
            )
            formatted.append(
                f"Inventory After Processing: {proc_data.get('inventory_minus_processing_count', 0)}"
            )
            formatted.append(
                f"Inventory After Staging: {proc_data.get('inventory_minus_staged_count', 0)}"
            )
        ...
        if context.get("inventory_minus_staged"):
            ...
            for i, item in enumerate(context["inventory_minus_staged"][:10]):  # Show first 10
                formatted.append(
                    f"{i+1}. Warehouse: {item.get('warehouse', 'N/A')} | SKU: {item.get('sku', 'N/A')} | Available: {item.get('available_balance', 'N/A')}"
                )
