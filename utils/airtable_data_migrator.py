    ...
    def migrate_delivery_services(self, file_path: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(0, total, batch_size):
                ...
                logger.info(f"Processing batch {i//batch_size + 1}/{(total-1)//batch_size + 1} ({len(batch)} records)")
                
                for service in batch:
                    # Check if record already exists to avoid duplicates
                    existing = self.airtable.get_delivery_services(
                        zip_prefix=service["destination_zip_short"],
                        origin=service["origin"]
                    )
                    
                    if existing:
                        logger.info(f"Delivery service already exists for zip {service['destination_zip_short']} from {service['origin']}")
                        continue
                    ...
                    try:
                        ...
                        logger.info(f"Created delivery service for zip {service['destination_zip_short']} from {service['origin']}")
            ...
            logger.info(f"Migration complete. Created {len(created_records)} new delivery service records")
    ...
    def migrate_fulfillment_centers(self, file_path: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for fc in fulfillment_centers:
                ...
                existing = [x for x in self.airtable.get_fulfillment_centers() if x.get("name") == fc["name"]]
            ...
            logger.info(f"Migration complete. Created or updated {len(created_records)} fulfillment center records")
            return created_records
        ...
        try:
            ...
            for fc in fulfillment_centers:
                ...
                for i in range(0, total, batch_size):
                    ...
                    logger.info(f"Processing batch {i//batch_size + 1}/{(total-1)//batch_size + 1} ({len(batch)} zones)")
                    
                    for zone_data in batch:
                        # Check if zone already exists
                        existing_zones = self.airtable.get_fulfillment_zones()
                        existing = [z for z in existing_zones if 
                                  z.get("zip_prefix") == zone_data["zip_prefix"]]
                        ...
                        if existing:
                            logger.info(f"Zone for zip prefix {zone_data['zip_prefix']} already exists, updating")
                            try:
                                record = self.airtable.update_fulfillment_zone(existing[0]["airtable_id"], zone_record)
                                created_records.append(record)
                            except Exception as e:
            ...
            logger.info(f"Fulfillment zones migration complete. Processed {len(created_records)} zone records")
            return created_records
        ...
        try:
            ...
            for warehouse, warehouse_data in data.items():
                ...
                for i in range(0, total_skus, batch_size):
                    ...
                    logger.info(f"Processing batch {i//batch_size + 1}/{(total_skus-1)//batch_size + 1} ({len(batch)} records)")
                ...
                for i in range(0, total_bundles, batch_size):
                    batch = items[i:i+batch_size]
                    logger.info(f"Processing bundle batch {i//batch_size + 1}/{(total_bundles-1)//batch_size + 1} ({len(batch)} records)")
                    ...
                    for bundle_sku, components in batch:
                        ...
                        bundle_data = {
                            ...
                            "warehouse": warehouse,
                            "is_bundle": True,
                            "bundle_components": json.dumps(components)  # Store components as JSON string
                        }
                        ...
                        try:
                            ...
                            logger.error(f"Failed to create bundle SKU mapping for {bundle_sku}: {e}")
