...
# Sheet IDs
SPREADSHEET_ID = '19-0HG0voqQkzBfiMwmCC05KE8pO4lQapvrnI_H7nWDY'
...
def get_credentials():
    ...
    try:
        ...
        raise Exception(f"Could not authenticate. Make sure you have either the JSON file locally or have run 'gcloud auth application-default login'")
...
def process_sku_data(data: List[List[Any]], center: str) -> Dict[str, Dict[str, Any]]:
            ...
            if len(group_rows) > 1:
                ...
                for row_idx, mapping in group_rows:
                    try:
                        ...
                        logger.warning(f"Error parsing bundle component for {order_sku} at row {row_idx}: {e}")
                
                if components_list:
                    result[center]["bundles"][order_sku] = components_list
                    logger.debug(f"Processed bundle {order_sku} with {len(components_list)} components")
            else:
                ...
                try:
                    ...
                    if not picklist_sku:
                        logger.warning(f"No picklist sku found for single SKU {order_sku}, using order SKU as fallback")
    ...
    logger.info(f"Processed {len(result[center]['singles'])} singles and {len(result[center]['bundles'])} bundles for {center}")
    return result
...
def load_sku_mappings_from_sheets(center=None):
    ...
    logger.info(f"Total loaded: {total_singles} singles and {total_bundles} bundles across all warehouses")
...
if __name__ == "__main__":
    try:
        ...
        print(f"Loaded {sum(len(result[warehouse]['singles']) for warehouse in result)} singles and {sum(len(result[warehouse]['bundles']) for warehouse in result)} bundles")
    except Exception as e:
