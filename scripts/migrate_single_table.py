...
def migrate_fulfillment_centers(handler: AirtableHandler, file_path: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"Migration complete. Created {len(created_records)} fulfillment center records")
...
def migrate_fulfillment_zones(handler: AirtableHandler, file_path: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for fc in fulfillment_centers:
            ...
            fc_record = next((record for record in fc_records if record.get("name") == fc_name), None)
        ...
        logger.info(f"Fulfillment zones migration complete. Created {len(created_records)} zone records")
        return created_records
    ...
    try:
        ...
        for fc_name, fc_data in data.items():
            ...
            for i in range(0, total_skus, batch_size):
                ...
                logger.info(f"Processing batch {i//batch_size + 1} of {(total_skus + batch_size - 1)//batch_size}")
                ...
                for sku_id, sku_data in batch:
                    try:
                        ...
                        sku_record = {
                            ...
                            "total_pick_weight": float(sku_data.get("Total_Pick_Weight", 0)),  # Use numeric value
                            "pick_type": sku_data.get("Pick_Type", ""),
                            "fulfillment_center": [fc_id]  # Link to fulfillment center using its ID in array format
                        }
                        ...
                        if sku_id in bundles:
                            ...
                            if bundle_components_data:
                                ...
                                logger.info(f"Added {len(bundle_components_data)} bundle components for {sku_id} as JSON string")
                        
                        # Create record in Airtable
                        record = handler.create_sku_mapping(sku_record)
                        stats["created"] += 1
                        stats["total"] += 1
...
def main():
    ...
    parser.add_argument('table', choices=['fulfillment_centers', 'fulfillment_zones', 'priority_tags', 'sku_mappings'],
