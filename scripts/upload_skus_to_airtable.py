...
def upload_sku_mappings(json_path: str = "/Users/olenaliubynetska/fulfillment-mixy-matchi/constants/data/sku_mappings.json") -> Dict[str, int]:
    """
    ...
    try:
        ...
        for warehouse_name, warehouse_data in data.items():
            ...
            if warehouse_name not in fc_map:
                logger.error(f"Fulfillment center '{warehouse_name}' not found in Airtable. Skipping.")
                continue
            ...
            for order_sku, mapping in all_skus.items():
                ...
                existing = next((m for m in sku_mappings if 
                               m.get("order_sku") == order_sku and 
                               m.get("picklist_sku") == mapping["picklist_sku"]), None)
            ...
            for order_sku, components in bundles.items():
                ...
                sku_data = {
                    ...
                    "total_pick_weight": float(first_component.get("weight", 0.0)) if first_component.get("weight") else None,
